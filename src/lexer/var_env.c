...
void    replace_env_variable(char **data, char *prefix, char *sufix, char *env_value)
{
    ft_memfree(*data);
    if (env_value)
    {
...
void process_env_substring(char **dollar_pos, char **str, char **sufix, char **env_value)
{
...
void    ft_replace(t_lexer **lexer)
{
    ...
    while (tmp) 
    {
        if (tmp->type == T_CMD || tmp->type == T_INFILE || tmp->type == T_OUTFILE)
        {
            if(tmp->data[0] == '\"') 
